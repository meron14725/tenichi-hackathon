...
✅ **確定済み** — `backend/docs/spec/データモデル.md` に全エンティティのリレーション設計が完成しています。
...
- タグ（デート・会食などTPO）
- 準備事項（メモ）
- 所属予定リスト（`schedule_list_id`、nullable）
- 選択ルート（ScheduleRoute、1予定につき1ルート）
...
| POST | `/routes/search` | 経路検索（複数ルート候補を返却） |

**ルート選択・保存（Schedule サブリソース）:**
...
> モバイルアプリが `/routes/search` でルート候補を取得 → ユーザーが1つ選択 → `POST /schedules/{id}/route` で保存するフロー。
...
1日の予定をまとめた名前付きリスト。カレンダー表示用。Template（再利用可能な雛形）とは別概念で共存。持ち物リスト（PackingItem）を内包する。

| メソッド | エンドポイント | 概要 |
|---------|--------------|------|
| GET | `/schedule-lists` | 予定リスト一覧取得（日付範囲でフィルタ） |
| POST | `/schedule-lists` | 予定リスト作成 |
| GET | `/schedule-lists/{id}` | 予定リスト詳細取得（所属予定 + 持ち物含む） |
| PUT | `/schedule-lists/{id}` | 予定リスト更新 |
| DELETE | `/schedule-lists/{id}` | 予定リスト削除 |
| POST | `/schedule-lists/{id}/packing-items` | 持ち物追加 |
| PUT | `/schedule-lists/{id}/packing-items/{item_id}` | 持ち物更新（チェック状態変更含む） |
...
全ユーザー共通のグローバルタグとして管理します（`backend/docs/spec/データモデル.md § A-1` 参照）。
...
> `POST /tags` は実装しない（`backend/docs/spec/データモデル.md § A-1, A-2` 参照）。
> リクエスト / レスポンスの詳細は `backend/docs/spec/API詳細設計.md` を参照。
