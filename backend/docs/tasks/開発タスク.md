...
- [ ] 🟡 `POST /routes/search` を実装する（経路検索・複数ルート候補返却）
...
- [ ] 🟡 `ScheduleRoute` モデルを作成しマイグレーションを実行する
- [ ] 🟡 `POST /schedules/{id}/route` を実装する（選択ルート保存・置換）
- [ ] 🟡 `GET /schedules/{id}/route` を実装する
- [ ] 🟡 `DELETE /schedules/{id}/route` を実装する
