...
- [ ] 🟡 `POST /templates/{id}/apply` を実装する（`date` を受け取り TemplateSchedule の時刻と組み合わせて Schedule を複数作成、ScheduleList も自動生成）

### ScheduleLists（予定リスト）

> 予定リストは「1日の予定をまとめた名前付きリスト」。カレンダー表示用。Template（再利用雛形）とは別概念。`ScheduleList` / `PackingItem` を実装する。

- [ ] 🟡 `ScheduleList` / `PackingItem` モデルを作成しマイグレーションを実行する
- [ ] 🟡 `Schedule` モデルに `schedule_list_id` カラムを追加するマイグレーションを実行する
- [ ] 🟡 `GET /schedule-lists` を実装する（クエリパラメータ: `start_date` / `end_date`）
- [ ] 🟡 `POST /schedule-lists` を実装する（packing_items 配列を一括作成）
- [ ] 🟡 `GET /schedule-lists/{id}` を実装する（所属予定 + 持ち物をネストして返す）
- [ ] 🟡 `PUT /schedule-lists/{id}` を実装する
- [ ] 🟡 `DELETE /schedule-lists/{id}` を実装する（持ち物カスケード削除、Schedule の `schedule_list_id` を null に）
- [ ] 🟡 `POST /schedule-lists/{id}/packing-items` を実装する
- [ ] 🟡 `PUT /schedule-lists/{id}/packing-items/{item_id}` を実装する
- [ ] 🟡 `DELETE /schedule-lists/{id}/packing-items/{item_id}` を実装する
