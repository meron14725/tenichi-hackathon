...
| Vertex AI (Gemini) | サービスアカウント (ADC) | Application Default Credentials |
...
| 変数名 | 用途 | 参照元 |
|--------|------|--------|
| `GCP_PROJECT_ID` | GCP プロジェクトID（Vertex AI 用） | fastapi-backend |
| `GCP_LOCATION` | GCP リージョン（デフォルト: asia-northeast1） | fastapi-backend |
