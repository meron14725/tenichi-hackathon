...
```json
{
  "summary": "<この変更が何のためのものか、1〜2文で簡潔に>",
  ...
  "comments": [
    {
      ...
      "comment": "<具体的な指摘内容>",
      "suggestion": "<修正後のコード（該当行の修正案）>"
    }
  ]
}
```

- commentsは指摘がなければ空配列 [] にしてください
- MUST: 必ず修正すべき問題（バグ、セキュリティ）
- WANT: 修正した方が良い（パフォーマンス、可読性）
- NITS: 細かい指摘（命名、スタイル）
- suggestion: 指摘に対する具体的な修正案がある場合、修正後のコードを記載してください。該当行をどう書き換えるべきかを示してください。修正案がない場合や、設計方針に関する指摘など具体的なコード修正が難しい場合はnullにしてください。
"""
...
def run_review(client: genai.Client, all_diffs: str, cache_name: str | None = None) -> tuple[dict, dict]:
    """Send all diffs to Gemini in a single API call. Returns (review_result, usage_stats)."""
    ...
    empty_result = {"summary": "", "good_points": [], "comments": []}

    try:
        result = json.loads(response.text)
    except json.JSONDecodeError:
        print("  Warning: Failed to parse JSON response, skipping.")
        return empty_result, usage
...
def main() -> None:
    ...
    try:
        ...
        result, usage = run_review(client, all_diffs, cache_name=cache.name if use_cache else None)
    ...
    # Overview
    if summary:
        summary_lines.append(f"### 変更概要\n{summary}\n")

    # Good points
    if good_points:
        summary_lines.append("### 良い点")
    ...
    # Review result
    if total_comments == 0:
        summary_lines.append("### レビュー結果\nLGTM 👍 指摘事項はありませんでした。\n")
    else:
        summary_lines.append(f"### レビュー結果\n{total_comments} 件の指摘をインラインコメントとして投稿しました。\n")
